...
SW = pd.read_excel('22-1 소웨 전공.xlsx')
prev = pd.read_excel('21-1 소웨 전공.xlsx')
past = pd.read_excel('20-1 소웨 전공.xlsx')

with open('졸작수강기록.csv', encoding='ISO-8859-1') as myfile:
...
def required_rec():
    data_df = pd.read_csv('졸작수강기록.csv', encoding='ISO-8859-1')
    ...
    result_df.to_csv('sgd.csv', header=False, index=False, encoding='utf-8-sig')
    ...
    result_fp.to_csv('fp.csv', header=False, index=False, encoding='utf-8-sig')
    relation = pd.read_csv('fp.csv')
...
# 키워드 csv파일 가져오기
csv = pd.read_csv('2020_1_전공_키워드.csv', encoding='cp949')
